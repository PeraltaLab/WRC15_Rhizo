...
rm(list=ls())
knitr::opts_knit$set(root.dir="~/GitHub/WRC15_Rhizo/data")
#knitr::knit("WRC2015RhizoDec2018.Rmd")
list.files()
```
...
data(otu.2, meta)
# core OTUs
core <- core.OTU.rank(data=list(Otu.2=otu.2), meta=meta, rank="g",
                     meta.factor="bulkM", percent=1)
...
group.venn(vectors=vectors, label=FALSE, cat.pos=c(330, 150),
           lab.cex=0.7, cex=3)
#Mowed: overlap bulk, av-rhizo, ec-rhizo
#All 4 plots sampled are combined for each treatment
#For each treatment: select treatment, remove meta, keep only present columns, paste into a list
metaotu.2.bulkM <- metaotu.2[which(metaotu.2$pxt=='bulkM'),]
metaotu.2.bulkM<-metaotu.2.bulkM[,-c(1:24)]
head(metaotu.2.bulkM)
metaotu.2.bulkM<-metaotu.2.bulkM[,colSums(metaotu.2.bulkM)>0]
bulkM<-paste(colnames(metaotu.2.bulkM))

metaotu.2.avM <- metaotu.2[which(metaotu.2$pxt=='avM'),]
metaotu.2.avM<-metaotu.2.avM[,-c(1:19)]
metaotu.2.avM<-metaotu.2.avM[,colSums(metaotu.2.avM)>0]
avM<-paste(colnames(metaotu.2.avM))

metaotu.2.ecM <- metaotu.2[which(metaotu.2$pxt=='ecM'),]
metaotu.2.ecM<-metaotu.2.ecM[,-c(1:19)]
metaotu.2.ecM<-metaotu.2.ecM[,colSums(metaotu.2.ecM)>0]
ecM<-paste(colnames(metaotu.2.ecM))
...
v<- venn.diagram(
x = list(bulkM , avM , ecM),
category.names = c("bulkM" , "avM " , "ecM"),
filename = '#mowed_venn_diagramm.png',
        output = TRUE ,
        imagetype="png" ,
        height = 480 , 
        width = 480 , 
